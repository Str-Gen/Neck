...
parser.add_argument("-Q", "--questions", action="store", dest="Q", help="Number of questions", type=int, default=10)
parser.add_argument("-T", "--time", action="store", dest="T", help="Time the responses, then restrict the answer time", type=bool, default=False)
parser.add_argument("-R", "--reverse", action="store", dest="R", help="Instead of notes -> numbers, numbers -> notes", type=bool, default=False)
parser.add_argument("-F", "--findall", action="store", dest="F", help="Find all instances along the neck", type=bool, default=False)
parser.add_argument("-C", "--chords", action="store", dest="C", help="Train chords")
# todo add sub args for chords Maj, Min, 7, Maj7, Min7, Dim, Aug, ...
parser.add_argument("-S", "--scales", action="store", dest="S", help="Train scales")
# todo add sub args for scales Maj Min and the modes, heptatonic, pentatonic, chromatic, dominant, natural
parsed_opts = parser.parse_args()

# 12 tone
complete = ["A", "A# / Bb", "B / Cb", "B# / C", "C# / Db", "D", "D# / Eb", "E / Fb", "E# / F", "F# / Gb", "G", "G# / Ab"]
simplified_sharps = ["A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#"]
simplified_flats = ["A", "Bb", "B", "C", "Db", "D", "Eb", "E", "F", "Gb", "G", "Ab"]
...
for tone in ("E","A","D","G","B","E"):
    ...
    seq = [simplified_sharps[i%len(simplified_sharps)] for i in range(idx,idx+13)]
...
for tone in ("E","A","D","G","B","E"):
    ...
    seq = [simplified_flats[i%len(simplified_flats)] for i in range(idx,idx+13)]
...
print(c.HEADER, "The quiz starts", c.ENDC)
tuning = ['E','A','D','G','B','E']
for x in prepare_questions():
    stringnr, fretnr, note  = x
    if not parsed_opts.R:
        print(c.WARNING,"Which fret is",note,"on string",tuning[stringnr],"?",c.ENDC)
        answer = input("Your answer: ")
        if int(answer) in (fretnr-12, fretnr, fretnr+12):
            print(c.OKGREEN,"+",c.ENDC)
        else:
            if fretnr < 12:
                print(c.FAIL,"-",c.ENDC, c.OKBLUE, "=>", fretnr, "and", fretnr+12, c.ENDC)
            else:
                print(c.FAIL,"-",c.ENDC, c.OKBLUE, "=>", fretnr, "and", fretnr-12, c.ENDC)
    else:
        print("")